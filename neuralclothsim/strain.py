...
from utils.data_io import get_plot_grid_tensor
...
def covariant_first_derivative_of_covariant_first_order_tensor(covariant_vectors: List[torch.Tensor], ref_geometry: ReferenceGeometry) -> List[torch.Tensor]:
    vpd = jacobian(torch.stack(covariant_vectors, dim=2), ref_geometry.curvilinear_coords)
...
def covariant_first_derivative_of_covariant_second_order_tensor(covariant_matrix: List[torch.Tensor], ref_geometry: ReferenceGeometry):
    phipd = jacobian(torch.stack(covariant_matrix, dim=2), ref_geometry.curvilinear_coords)
...
def compute_strain(deformations: torch.Tensor, ref_geometry: ReferenceGeometry, nonlinear_strain: bool = True) -> Strain: 
    ...
    u_3pd = jacobian(u_3[...,None], ref_geometry.curvilinear_coords)
    ...
    '''
    fig_epsilon = get_plot_grid_tensor(epsilon_1_1, epsilon_1_2, epsilon_1_2, epsilon_2_2)
    fig_kappa = get_plot_grid_tensor(kappa_1_1, kappa_1_2, kappa_1_2, kappa_2_2)
    fig_epsilon.savefig(f'logs/epsilon.png')
    fig_kappa.savefig(f'logs/kappa.png')
    '''
